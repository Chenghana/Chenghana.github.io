---
title: TS中的数据类型
date: 2022-09-03 13:56:51
permalink: /pages/e7d557/
author: 
  name: niko
...
- string类型：一个字符系列，使用单引号（**''**）或双引号（**""**）来表示字符串类型。反引号（**``**）来定义多行文本和内嵌表达式(ES6模板字符串)
...
/* 
  数组是我们前端开发过程中，最常用的引用类型之一,在发送请求获取响应时(axios)，我们往往会使用到数组类型，因此我们务必要掌握好数组的几种定义方式
*/
```
...
第一种:
let goddess = ["邱淑贞", "赵雅芝"];
let [first, second,third] = goddess;
...
第三种:
//axios [code:xxx,aaa:111,nbbb:xxx,data:XXX]
